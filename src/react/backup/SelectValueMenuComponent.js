...
import AddValue from './AddValueComponent';
...
class SelectValueMenu extends Component {
    state={anchor:null, label:"",add:null};
    constructor(props) {
	super(props);
	this.keydown=(e) => {
	    if (this.state.add !== null) {
		var label=this.state.label;
		if (e.keyCode === 8 || e.keyCode === 46) { //backspace or delete
		    if (label.length > 0) {
			this.setState({label:label.slice(0,-1)});
		    };
		} else if (e.keyCode === 9 || e.keyCode === 32) { // tab, space
		    this.setState({label:label+" "});
		} else if (e.keyCode >= 48) {
		    this.setState({label:label+e.key});
		}
		//console.log("Pressed key:",e.keyCode,":",this.state.label);
	    };
	};
	...
	if (items.length > 0) { // items
		...
		onKeyDown= {this.keydown}
		    ...
		    <AddValue state={state} keyitem={keyitem}
		label={this.state.label} add={this.state.add} setState={this.setState.bind(this)}/>
		    </MenuItem>
