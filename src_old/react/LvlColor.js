...
class LvlColor extends Component {
    state = {anchor: null};
    constructor() {
	super();
...
//	}
    ...
    render() {
        const { state,classes,bg,fg,lvl } = this.props;
	//console.log("Rendering LvlColor...");
	this.onClick = (event) => {this.open(event)};//console.log("Color:",hex,lvl);
	const handleColorChange = ({ hex }) => {state.Colors.setLvlBgColor(state,lvl,hex);state.Show.showAll(state,false,true);}
...
export default withStyles(styles)(LvlColor);
