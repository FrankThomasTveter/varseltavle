...
function renderItem(classes,state,bg,fg,lvl,cnt) {
    //var plans=state.Layout.makePlans();
    //console.log("Showing palette:",bg,fg,JSON.stringify(plans.hd2));
    return (<LvlColor key={lvl} state={state} bg={bg} fg={fg} lvl={lvl} classes={{divTableCell:classes.divTableCell}}/>);
...
class LvlBar extends Component {
    constructor(props) {
        super(props);
        const {state} = props;
        state.React.LvlBar=this;
    };
    showLvlBar(state) {
	//console.log("Called LvlBar.show...");
	this.forceUpdate();
    };
    render() {
...
export default withStyles(styles)(LvlBar);
